...
util.invokeRequest = (param) => {
  return new Promise(function (resolve, reject) {
    ...
    if (param.httpHeaders != undefined) {
      ...
      for (const key in param.httpHeaders) {
        req = req.set(key, param.httpHeaders[key])
      }
    }

    if ((param.httpMethod == 'POST' || param.httpMethod == 'PUT') && param.formData != undefined) {
      const postData = qs.stringify(param.formData, null, null, { encodeURIComponent: encodeURIComponent })
    ...
    if ((param.httpMethod == 'POST' || param.httpMethod == 'PUT') && param.jsonData != undefined) {
      if (param.base64Data != undefined) {
        const buff = fs.readFileSync(param.base64Data.dataFileName)
        const base64data = buff.toString('base64')
        param.jsonData[param.base64Data.fieldName] = base64data
      }

      const postData = JSON.stringify(param.jsonData)
    ...
    if ((param.httpMethod == 'POST' || param.httpMethod == 'PUT') && param.binaryData != undefined) {
      ...
      if (param.binaryData.dataFileName != undefined) { postData = fs.readFileSync(param.binaryData.dataFileName) }
...
util.getApexSecurityToken = (param) => {
  ...
  if (param.nextHop != undefined && param.nextHop.signatureUrl != undefined) {
    ...
    const childToken = apex.getSignatureToken(param.nextHop)
...
function skipTestCheck (param) {
  ...
  if (param.skipTest != undefined && param.skipTest) { return true } else { return false }
}
  ...
  return new Promise(function (resolve, reject) {
    ...
    return new Promise(function (resolve) {
      ...
      return util.invokeRequest(param).then(function (res) {
        if (!_.isEmpty(res.body)) { param.responseBody = res.body } else { param.responseText = res.text }
        ...
        if (message != undefined && message.trim().length > 0) { childMessage += indentation + message.replace(/\n/g, '\n' + indentation) }
    ...
    //     }
      ...
      .catch(function (error) {
        ...
        if (childMessage.trim().length > 0) {
          if (message.trim().length > 0) { message += childMessage } else { message = childMessage }
        }
